...
export interface ChatMessage {
  ...
  export class AnalyzeWhatsapp {
    ...
    private detectType(message: string): string {
      ...
      if (msgLower.startsWith("location: https://maps.google.com/")) return "location";
      ...
      if (msgLower.includes("audio omitted") || msgLower.endsWith(".m4a document omitted")) return "audio";
      ...
      if (msgLower.includes("this message was deleted") || msgLower.includes("you deleted this message"))
        return "deleted message";
      if (msgLower.includes("turned on disappearing messages") || msgLower.includes("turned off disappearing messages"))
        return "disappearing message notification";
      if (msgLower.includes("messages and calls are end-to-end encrypted")) return "encryption notice";
      if (msgLower.includes("only messages that mention @meta ai")) return "meta ai notice";
    ...
    private jsonize(data: { [key: string]: number }, labels: [string, string] = ["key", "count"]): any[] {
    ...
    public mostFreqWordsPerAuthor(author: string): { word: string; count: number }[] {
      const counts: { [word: string]: number } = {};
      this.messages
    ...
    public laughsPerAuthor(): { author: string; laugh_count: number }[] {
      ...
      this.messages.forEach((msg) => {
        ...
        const count = (haMatches ? haMatches.length : 0) + (kMatches ? kMatches.length : 0);
    ...
    public laughsLengthPerAuthor(): { author: string; laugh_length_ha: number; laugh_length_k: number }[] {
      const results: {
        [author: string]: { totalHa: number; countHa: number; totalK: number; countK: number };
      } = {};
